...
class NumberRepository @Inject constructor(
    ...
    private val numberApi: NumberApi
     ...
     suspend fun searchNumber(number: String, type: NumberType, isRandom: Boolean):Response<NumberItem>{
        return try {
            val result = numberApi.getNumber(if (isRandom) "random" else number, type.name.lowercase())
            if (result.isEmpty()){
                Response.Error("")
            } else {
                val newNumber = if (isRandom) result.split(" ").first() else number
    ...
    suspend fun getNumbersFromDB() = dao.getAllNumbers()
}